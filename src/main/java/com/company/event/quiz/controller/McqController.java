...
public class McqController {
        ...
        public ResponseEntity<?> start(@PathVariable String eventId,
                ...
                return ResponseEntity.ok(mcqService.startTest(studentId, eventId, pin));
        }
        ...
        public ResponseEntity<byte[]> exportAnalyticsPdf(@PathVariable String eventId) {
                ...
                AdminEventAnalyticsDTO analytics = mcqService.getEventAnalytics(eventId);

                Event event = eventRepository.findById(eventId)
                                .orElseThrow(() -> new RuntimeException("Event not found"));

                ByteArrayInputStream pdfStream = pdfExportService.generateAnalyticsPdf(analytics, event.getTitle());
        ...
        public ResponseEntity<?> getResult(@PathVariable String eventId,
                        @RequestHeader("studentId") String studentId) {
                return ResponseEntity.ok(mcqService.getStudentResult(studentId, eventId));
        }

        @GetMapping("/history")
        public ResponseEntity<?> getHistory(@RequestHeader("studentId") String studentId) {
                return ResponseEntity.ok(mcqService.getStudentHistory(studentId));
        }
