            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                ...
                {!showForm && (
                    <button
                        ...
                        className="btn-primary bg-purple-600 hover:bg-purple-700 shadow-purple-200 flex items-center space-x-2"
                    >
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="card p-8 space-y-8 glass-effect border-purple-100 shadow-2xl">
                            <h3 className="text-2xl font-black text-gray-800 border-l-4 border-purple-500 pl-4 tracking-tight">Main Specifications</h3>
                            ...
                            <div className="flex justify-end pt-4 border-t border-gray-100">
                                <label className="cursor-pointer text-sm font-bold text-purple-600 hover:text-purple-700 flex items-center space-x-2 bg-purple-50 px-4 py-2 rounded-lg border border-purple-100 transition-all hover:shadow-md">
                        ...
                        <div className="card p-8 space-y-6 overflow-hidden">
                            <div className="flex justify-between items-center">
                                <h3 className="text-2xl font-black text-gray-800">Test Cases</h3>
                            ...
                            <div className="space-y-6">
                                {formData.testCases.map((tc, idx) => (
                                    <div key={idx} className="p-6 rounded-3xl bg-gray-50/50 border border-gray-100 space-y-4 relative group">
                                        <div className="absolute top-4 right-4 flex items-center space-x-4">
                                            <label className="flex items-center space-x-2 cursor-pointer">
                                                ...
                                                <span className="text-[10px] font-black uppercase text-gray-400">Hidden</span>
                                            </label>
                                            <button type="button" onClick={() => setFormData({ ...formData, testCases: formData.testCases.filter((_, i) => i !== idx) })} className="text-gray-300 hover:text-red-500 transition-colors">
                                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="space-y-2">
                                                <span className="text-[10px] font-black uppercase text-gray-400">Input</span>
                                                <textarea className="w-full bg-white border border-gray-100 rounded-xl p-3 font-mono text-xs h-24 resize-none outline-none focus:border-purple-300 transition-all" value={tc.input} onChange={(e) => updateTestCase(idx, 'input', e.target.value)} />
                                            </div>
                                            <div className="space-y-2">
                                                <span className="text-[10px] font-black uppercase text-gray-400">Expected Output</span>
                                                <textarea className="w-full bg-white border border-gray-100 rounded-xl p-3 font-mono text-xs h-24 resize-none outline-none focus:border-purple-300 transition-all" value={tc.expectedOutput} onChange={(e) => updateTestCase(idx, 'expectedOutput', e.target.value)} />
                                            </div>
                                        </div>
                                    </div>
                                ))}
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="card p-8 space-y-6 sticky top-28 shadow-3xl">
                            ...
                            <div className="flex flex-col gap-4 pt-4">
                                <button type="submit" className="w-full py-5 bg-purple-600 text-white rounded-[2rem] font-black text-xl hover:bg-purple-700 transition-all shadow-xl shadow-purple-100 active:scale-95">
                                    {editingProblem ? 'Save Changes' : 'Publish Problem'}
                                </button>
                                <button type="button" onClick={() => setShowForm(false)} className="w-full py-4 bg-gray-50 text-gray-400 rounded-[2rem] font-bold text-sm hover:bg-gray-100">
                                    Discard Draft
                                </button>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        ...
                        <div key={prob.id} className="card p-8 group hover:shadow-2xl transition-all border-gray-100 flex flex-col transform hover:-translate-y-2">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <div className="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleEdit(prob)} className="p-2.5 text-gray-400 bg-gray-50 hover:text-purple-600 rounded-xl"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                                    <button onClick={() => handleDelete(prob.id)} className="p-2.5 text-gray-400 bg-gray-50 hover:text-red-500 rounded-xl"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                </div>
                            </div>
                            <h3 className="text-2xl font-black text-gray-800 mb-4 line-clamp-2">{prob.title}</h3>
                            <p className="text-gray-500 text-sm font-medium line-clamp-3 mb-8 leading-relaxed flex-grow">{prob.description}</p>
                            <div className="flex justify-between items-center pt-6 border-t border-gray-50">
                                <div className="flex items-center space-x-2">
                                    <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 font-black text-xs">{(prob.testCases || []).length}</div>
                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Test Cases</span>
                                </div>
                    ...
                    {problems.length === 0 && (
                        <div className="col-span-full py-24 text-center bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
                            <p className="text-gray-400 font-black text-xl mb-4">The Repository is Empty</p>
                            <button onClick={() => setShowForm(true)} className="btn-primary bg-purple-600 shadow-purple-100">Initialize Problem Set</button>
                        </div>
                    )}
                </div>
            )
